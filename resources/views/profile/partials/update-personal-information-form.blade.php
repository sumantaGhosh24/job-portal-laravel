<section>
    ...
    <form method="post" action="{{ route('profile.update.personal') }}" class="mt-6 space-y-6">
        ...
        <div>
            ...
            <x-text-input id="desired_job_title" name="desired_job_title" type="text" class="mt-1 block w-full" :value="old('desired_job_title', $user->desired_job_title)" required autofocus autocomplete="desired_job_title" />
        ...
        <div>
            ...
            <select id='desired_job_type' name='desired_job_type' class="mt-2 w-full px-4 py-2 rounded-md border border-gray-300" value="{{ old('desired_job_type') }}" required autofocus autocomplete="desired_job_type">
                <option value="">Select Desired Job Type</option>
                <option value="full-time" {{ $user->desired_job_type == 'full-time' ? 'selected' : '' }}>Full-time</option>
                <option value="part-time" {{ $user->desired_job_type == 'part-time' ? 'selected' : '' }}>Part-time</option>
                <option value="contract" {{ $user->desired_job_type == 'contract' ? 'selected' : '' }}>Contract</option>
                <option value="internship" {{ $user->desired_job_type == 'internship' ? 'selected' : '' }}>Internship</option>
                <option value="temporary" {{ $user->desired_job_type == 'temporary' ? 'selected' : '' }}>Temporary</option>
            </select>
