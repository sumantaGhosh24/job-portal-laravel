    ...
    <div class="mt-6">
        <form method="post" action="{{ route('profile.language.add') }}" class="space-y-6">
            ...
            <div>
                ...
                <select id='proficiency' name='proficiency' class="mt-2 w-full px-4 py-2 rounded-md border border-gray-300" value="{{ old('proficiency') }}" required autofocus autocomplete="proficiency">
        ...
        <div class="flex justify-between gap-3 flex-wrap">
            @forelse ($user->languages as $language)
                <form method="post" action="{{ route('profile.language.update', ['id' => $language->id]) }}" class="space-y-6">
                    ...
                    <div>
                        <x-input-label for="name" :value="__('Language Name')" />
                        <x-text-input id="name" name="name" type="text" class="mt-1 block w-full" :value="old('name', $language->name)" required autofocus autocomplete="name" />
                        <x-input-error class="mt-2" :messages="$errors->get('name')" />
                    </div>
                    <div>
                        <x-input-label for="proficiency" :value="__('Language Proficiency')" />
                        <select id='proficiency' name='proficiency' class="mt-2 w-full px-4 py-2 rounded-md border border-gray-300" value="{{ old('proficiency') }}" required autofocus autocomplete="proficiency">
                            <option value="">Select Language Proficiency</option>
                            <option value='elementary' {{ $language->proficiency == 'elementary' ? 'selected' : '' }}>Elementary</option>
                            <option value='limited-working' {{ $language->proficiency == 'limited-working' ? 'selected' : '' }}>Limited Working</option>
                            <option value='professional-working' {{ $language->proficiency == 'professional-working' ? 'selected' : '' }}>Professional Working</option>
                            <option value='full-professional' {{ $language->proficiency == 'full-professional' ? 'selected' : '' }}>Full Professional</option>
                            <option value='native-or-bilingual' {{ $language->proficiency == 'native-or-bilingual' ? 'selected' : '' }}>Native or Bilingual</option>
                        </select>
                        <x-input-error class="mt-2" :messages="$errors->get('proficiency')" />
                    </div>
    ...
    <div class="mt-6">
        ...
        @forelse ($user->languages as $language)
            <form method="post" action="{{ route('profile.language.destroy', ['id' => $language->id]) }}" class='flex items-center justify-between gap-3 border border-blue-500 px-1 py-1.5 rounded-md mb-3'>
                ...
                <x-danger-button>{{ __('Delete Language') }}</x-danger-button>
            </form>
        @empty
            <p class='text-lg font-bold'>No languages</p>
        @endforelse
