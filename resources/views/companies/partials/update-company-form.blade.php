<section>
    <div>
        ...
        <p class="mt-1 text-lg text-gray-800">{{ __("Update company information") }}</p>
    </div>
    ...
    <form method="post" action="{{ route('companies.information', ['id' => $company->id]) }}" class="mt-6 space-y-6">
        ...
        <div>
            ...
            <select id='sector' name='sector' class="mt-2 w-full px-4 py-2 rounded-md border border-gray-300"
                value="{{ old('sector') }}" required autofocus autocomplete="sector">
        ...
        <div>
            ...
            <select id='size' name='size' class="mt-2 w-full px-4 py-2 rounded-md border border-gray-300"
        ...
        <div>
            ...
            <x-text-input id="email" type="email" name="email" :value="old('email', $company->email)" required autofocus
                autocomplete="email" />
            <x-input-error :messages="$errors->get('email')" class="mt-2" />
        </div>
        <div>
            <x-input-label for="location" :value="__('Company Location')" />
            <x-text-input id="location" type="text" name="location" :value="old('location', $company->location)"
