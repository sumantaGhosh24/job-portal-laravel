<section class="max-w-4xl mx-auto">
    <div class="relative">
        ...
        <div class="absolute left-1/2 sm:left-12 -bottom-14 transform -translate-x-1/2 sm:translate-x-0">
            @isset($company->logo)
                <img src="{{ asset('storage/' . $company->logo) }}" alt="logo" class="w-28 h-28 sm:w-32 sm:h-32 rounded-full border-4 border-white shadow-lg object-cover bg-white dark:shadow-gray-300" />
            @else
    ...
    <div class="px-6 text-center sm:text-left">
        ...
        @if($member->role)
            <p class="text-base">
                {{ $member->position ?? 'N/A' }} - {{ $member->department ?? 'N/A' }} - {{ $member->status }} - {{ $member->role }} - {{ $member->joining_date }} - {{ $member->leaving_date ?? 'Present' }}
            </p>
        @else
        ...
        <div class="space-y-3">
            @forelse($company->followers as $followers)
                <div class="flex items-center justify-between bg-gray-50 p-3 rounded-lg dark:bg-gray-700">
                    ...
                    <a href="{{ route('profile', ['id' => $followers->id]) }}" class="text-blue-500 hover:underline">View</a>
                </div>
            @empty
        ...
        <div class="px-6 py-4 border-t border-gray-100 bg-gray-50 dark:bg-gray-700">
            ...
            <div class="flex items-center gap-3 flex-wrap">
                ...
                @if ($company->website_url)
                    <a href="{{ $company->website_url }}" target="_blank" class="text-blue-600 hover:underline">Personal Website</a>
                @endif
            </div>
        </div>
    @endif
