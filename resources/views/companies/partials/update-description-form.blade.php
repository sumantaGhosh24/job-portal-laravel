<section>
    <div>
        ...
        <p class="mt-1 text-lg text-gray-800">{{ __("Update company description") }}</p>
    </div>
    ...
    <form method="post" action="{{ route('companies.description', ['id' => $company->id]) }}" class="mt-6 space-y-6">
        ...
        <div>
            ...
            <textarea id="description" name="description"
                ...
                autocomplete="description">{{ old('description', $company->description) }}</textarea>
            <x-input-error class="mt-2" :messages="$errors->get('description')" />
        </div>

        <div>
            <x-input-label for="slogan" :value="__('Company Slogan')" />
            <textarea id="slogan" name="slogan" class="mt-1 block w-full px-4 py-2 rounded-md border border-gray-300"
                required autofocus autocomplete="slogan">{{ old('slogan', $company->slogan) }}</textarea>
