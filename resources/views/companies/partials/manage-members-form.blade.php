    ...
    <div class="mt-6">
        <form method="post" action="{{ route('members.store', ['id' => $company->id]) }}" class="space-y-6">
            ...
            <div>
                ...
                <x-text-input id="user_id" name="user_id" type="text" class="mt-1 block w-full" :value="old('user_id')" required autofocus autocomplete="user_id" />
            ...
            <div>
                ...
                <x-text-input id="department" name="department" type="text" class="mt-1 block w-full" :value="old('department')" required autofocus autocomplete="department" />
    ...
    <div class="mt-6">
        ...
        <div class="">
            @forelse ($company->members as $member)
                <form method="post" action="{{ route('members.update', ['id' => $company->id, 'memberId' => $member->id]) }}" class="flex items-center justify-between gap-3">
                    @csrf
                    @method('patch')
                    <div>
                    ...
                    <div>
                        <x-input-label for="position" :value="__('Member Position')" />
                        <x-text-input id="position" name="position" type="text" class="mt-1 block w-full" :value="old('position', $member->position)" required autofocus autocomplete="position" />
                        <x-input-error class="mt-2" :messages="$errors->get('position')" />
                    </div>
                    <div>
                        <x-input-label for="department" :value="__('Member Department')" />
                        <x-text-input id="department" name="department" type="text" class="mt-1 block w-full" :value="old('department', $member->department)" required autofocus autocomplete="department" />
                        <x-input-error class="mt-2" :messages="$errors->get('department')" />
                    </div>
            ...
            @if($member->role === 'admin' || $member->role === 'ex-employee')
                <div class='flex items-center justify-between gap-3 border border-blue-500 px-1 py-1.5 rounded-md mb-3'>
                    <h3 class='text-lg font-bold capitalize'>{{ substr($member->user->first_name ?? 'U', 0, 1) }}.
                        {{ $member->user->last_name }} - {{ $member->position ?? 'N/A' }} - {{ $member->department ?? 'N/A' }} - {{ $member->status }} - {{ $member->role }} - {{ $member->joining_date }} - {{ $member->leaving_date ?? 'Present' }}
                    </h3>
                </div>
            @else
                <form method="post" action="{{ route('members.remove', ['id' => $member->id]) }}" class='flex items-center justify-between gap-3 border border-blue-500 px-1 py-1.5 rounded-md mb-3'>
                    @csrf
                    @method('delete')
                    <h3 class='text-lg font-bold capitalize'>{{ substr($member->user->first_name ?? 'U', 0, 1) }}.
                        {{ $member->user->last_name }} - {{ $member->position ?? 'N/A' }} - {{ $member->department ?? 'N/A' }} - {{ $member->status }} - {{ $member->role }} - {{ $member->joining_date }} - {{ $member->leaving_date ?? 'Present' }}
