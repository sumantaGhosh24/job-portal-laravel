<section>
    ...
    <form method="post" action="{{ route('companies.banner', ['id' => $company->id]) }}" class="mt-6 space-y-6" enctype="multipart/form-data">
        @csrf
        @method('patch')
        <div>
            @if($company->banner)
        ...
        <div>
            ...
            <x-text-input id="image" name="image" type="file" class="mt-1 w-full px-4 py-2 rounded-md border border-gray-300 cursor-pointer" :value="old('image', $company->banner)" required autofocus autocomplete="image" />
