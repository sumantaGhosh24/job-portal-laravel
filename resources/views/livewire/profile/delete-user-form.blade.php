    ...
    <div>
        <form wire:submit="destroy">
            ...
            <div class="mt-6">
                ...
                <x-text-input id="password" wire:model="password" type="password" class="mt-1 block w-3/4" required />
                @error('password') <span class="text-lg text-red-600 mt-2">{{ $message }}</span> @enderror
            </div>
            <div class="mt-6 flex justify-end">
                <x-danger-button type="submit" class="ms-3">{{ __('Delete Account') }}</x-danger-button>
            </div>
        </form>
    </div>
</section>