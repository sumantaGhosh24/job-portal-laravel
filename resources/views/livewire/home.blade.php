        ...
        <div class="space-y-4">
            @forelse ($suggested as $user)
                <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg dark:bg-gray-700"
                    wire:key="{{ $user->id }}">
                    <div>
                        <h3 class="font-semibold text-gray-800 dark:text-white">{{ $user->first_name }}
                            {{ $user->last_name }}
                        </h3>
                        <p class="text-sm text-gray-500 dark:text-white">{{ '@' . Str::slug($user->username) }}</p>
                    </div>
                    <x-primary-button wire:click="followUser({{ $user->id }})">{{ __('Follow') }}</x-primary-button>
                </div>
            @empty
            ...
            @forelse ($suggestedCompanies as $company)
                <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg dark:bg-gray-700"
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <x-primary-button
                            wire:click="followCompany({{ $company->id }})">{{ __('Follow') }}</x-primary-button>
                    </div>
                </div>
            @empty
