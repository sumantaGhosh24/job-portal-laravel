        ...
        @if($job->is_active)
            <div class="space-y-5">
                ...
                <div>
                    ...
                    <form method="post" action="{{ route('applications.store', ['id' => $job->id]) }}" enctype="multipart/form-data">
                        @csrf
                        <div>
                            <x-input-label for="resume" :value="__('Resume')" />
                            <x-text-input id="resume" name="resume" type="file" class="mt-1 w-full px-4 py-2 rounded-md border border-gray-300 cursor-pointer" :value="old('resume')" required autofocus autocomplete="resume" />
                            <x-input-error class="mt-2" :messages="$errors->get('resume')" />
                        </div>
                        <div>
                            <x-input-label for="cover_letter" :value="__('Cover Letter')" />
                            <textarea id="cover_letter" name="cover_letter" class="mt-1 block w-full px-4 py-2 rounded-md border border-gray-300" required autofocus autocomplete="cover_letter">{{ old('cover_letter') }}</textarea>
                            <x-input-error class="mt-2" :messages="$errors->get('cover_letter')" />
                        </div>
                        <x-primary-button class="mt-5">{{ __('Submit Application') }}</x-primary-button>
                    </form>
                </div>
            </div>
        @else
