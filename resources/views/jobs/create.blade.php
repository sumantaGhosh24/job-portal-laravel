    ...
    @if(auth()->id() === $company->owner_id)
        <div class="flex justify-center items-center h-full">
            <div class="rounded-lg shadow-md p-8 shadow-black w-[75%] my-10 dark:shadow-gray-300">
                ...
                <form method="POST" action="{{ route('jobs.store', ['id' => $company->id]) }}">
                    @csrf
                    <div class="mt-4">
                        <x-input-label for="title" :value="__('Job Title')" />
                        <x-text-input id="title" type="text" name="title" :value="old('title')" required ="title" />
                    ...
                    <div class="mt-4">
                        ...
                        <x-text-input id="location" type="text" name="location" :value="old('location')" required autocomplete="location" />
                    ...
                    <div class="mt-4">
                        ...
                        <x-text-input id="salary" type="text" name="salary" :value="old('salary')" required autocomplete="salary" />
                        <x-input-error :messages="$errors->get('salary')" class="mt-2" />
                    </div>
                    <div class="mt-4">
                        <x-input-label for="deadline" :value="__('Job Deadline')" />
                        <x-text-input id="deadline" type="date" name="deadline" :value="old('deadline')" required autocomplete="deadline" />
                        <x-input-error :messages="$errors->get('deadline')" class="mt-2" />
                    </div>
                    <x-primary-button class="mt-5">{{ __('Create Job Post') }}</x-primary-button>
                </form>
            </div>
        </div>
    @else
